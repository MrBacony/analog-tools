...
export function detectProvider(userInfo: GenericUserInfo): IdentityProvider {
  ...
  if (userInfo["iss"] && typeof userInfo["iss"] === 'string' && userInfo["iss"].includes('auth0.com')) {
    return 'auth0';
  }

  // Keycloak typically includes an issuer URL with "auth/realms"
  if (userInfo["iss"] && typeof userInfo["iss"] === 'string' && userInfo["iss"].includes('/auth/realms')) {
    return 'keycloak';
  }
...
export function transformUserFromProvider(userInfo: GenericUserInfo): AuthUser {
  ...
  switch (provider) {
    ...
    default:
      ...
      return {
        username: userInfo["preferred_username"] || userInfo["nickname"] || userInfo["email"] || userInfo["sub"] || '',
