...
describe('Logger Type Safety', () => {
  ...
  describe('Runtime validation', () => {
    it('should warn and fallback to info for invalid log levels', () => {
      ...
      // Should have fallen back to info level
    ...
    it('should handle case insensitive input with warning', () => {
      const logger = new LoggerService({ 
        level: 'INFO' as LogLevel, // Uppercase
        name: 'test-logger' 
      });

      // Should have warned about invalid level (case sensitive)
      expect(mockConsole.warn).toHaveBeenCalledWith(
        expect.stringContaining('[LoggerService] Invalid log level "INFO"')
      );
  ...
  describe('Backwards compatibility concerns', () => {
    it('should handle environment variable LOG_LEVEL with validation', () => {
      ...
      new LoggerService({ name: 'env-test-invalid' });
      
      // Should have warned and fallen back to info
      expect(mockConsole.warn).toHaveBeenCalledWith(
        expect.stringContaining('[LoggerService] Invalid log level "invalid"')
      );
