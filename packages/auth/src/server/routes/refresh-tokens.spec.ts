...
describe('refresh-tokens route', () => {
  ...
  it('should refresh tokens when properly authenticated', async () => {
    ...
    expect(mockContextLogger.info).toHaveBeenCalledWith('Token refresh job completed', {
      refreshed: 5,
      failed: 1,
      total: 6
    });
  ...
  it('should throw 401 error when API key is missing', async () => {
    ...
    expect(mockContextLogger.warn).toHaveBeenCalledWith('Unauthorized token refresh attempt');
  ...
  it('should handle errors when refreshing tokens', async () => {
    ...
    mockAuthService.refreshExpiringTokens = vi.fn().mockRejectedValue(testError);
    ...
    expect(mockContextLogger.error).toHaveBeenCalledWith('Error in token refresh job', testError);
  });
});
