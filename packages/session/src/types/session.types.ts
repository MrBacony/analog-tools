 ...
 */
export type TTL<T extends Record<string, unknown> = Record<string, unknown>> =
  | number
  | ((data: T) => number);
 ...
 */
export interface UnstorageSessionStoreOptions<
...
export interface SessionStore<T extends SessionDataT = SessionDataT> {
  ...
  all?: () => Promise<Record<string, RawSession<T>>>;
